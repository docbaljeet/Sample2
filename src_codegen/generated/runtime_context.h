...
inline MortalityContext& active_mortality() {
    if (!g_mortality_ctx)
        throw std::runtime_error("No active mortality context");
    return *g_mortality_ctx;
}

inline LapseContext& active_lapse() {
    if (!g_lapse_ctx)
        throw std::runtime_error("No active lapse context");
    return *g_lapse_ctx;
}

inline EiaCreditContext& active_eia_credit() {
    if (!g_eia_credit_ctx)
        throw std::runtime_error("No active eia_credit context");
