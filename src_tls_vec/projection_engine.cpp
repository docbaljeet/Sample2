...
void ProjectionEngine::run(int projection_months, const ScriptHooks& hooks) {
    for (const auto& pol : policies_) {
        for (const auto& scn : scenarios_) {
            ...
            // ── Call each script ONCE per (policy, scenario) ─────
            // Scripts fill vectors of rates for all months.
            ...
            // ── Now the fast inner loop — pure C++, no Python ───
            ...
            for (int m = 0; m < projection_months; ++m) {
                ...
                // Print every 6 months (m is 0-based, display 1-based)
                if ((m + 1) % 6 == 0) {
